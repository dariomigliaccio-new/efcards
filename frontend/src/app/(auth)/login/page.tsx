    ...
    <div className="min-h-screen flex items-center justify-center px-6"
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="w-full max-w-sm opacity-0">
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-3xl mt-4 mb-2">Welcome back.</h1>
          <p className="text-sm text-dim">Sign in to your collection</p>
        </div>

        {/* Google OAuth */}
        <a
          href={`${API}/auth/google`}
          className="flex items-center justify-center gap-3 btn btn-ghost w-full mb-3 text-xs"
        >
        ...
        <p className="text-center text-xs text-dim mt-6">
          No account?{' '}
          <Link href="/register" className="text-gold hover:underline">Create one free</Link>
        </p>
      </form>
    </div>
  );
